...
#ifdef DEBUG
#   define LNRxSC_LOG(fmt, ...) NSLog((@"%s [Line %d] " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
#else
#   define LNRxSC_LOG(...)
#endif
...
+ (LNStompFrame *)fromString:(NSString *)string;

@end

@interface LNStompMessage()
...
+ (LNStompMessage *)fromFrame:(LNStompFrame *)frame
                        client:(LNStompClient *)client;

@end

@interface LNStompSubscription()
...
@property (nonatomic, strong) id<LNStompSubscriptionIdGenerator> idGenerator;

// LNStompSubscription object for each destination
...
- (id)initWithURL:(NSURL *)url {    
    return [[self initWithSocket:[[SRWebSocket alloc] initWithURL:[self convertToSockJsURL:url]]] useSockJs];
}
...
- (instancetype)subscriptionIdGenerator:(id<LNStompSubscriptionIdGenerator>)idGenerator {
...
- (RACSignal *)stompFrames
{
    return [[self.socketSubject
             ...
             map:^id(id value) {
                 return [LNStompFrame fromString:value];
             }];
}
...
- (RACSignal *)stompMessages
{
    ...
    return [[[self stompFrames]
              filter:^BOOL(LNStompFrame *frame) {
                  // only interested in STOMP "MESSAGE" frame
                  return [kCommandMessage isEqualToString:frame.command];
              }]
              map:^id(LNStompFrame *frame) {
                  @strongify(self)
                  return [LNStompMessage fromFrame:frame client:self];
              }];
}
    ...
    return [RACSignal
        createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
            ...
            @synchronized(_subscriptions) {
                LNStompSubscription *subscription = [_subscriptions objectForKey:destination];
                if (!subscription) {
                    LNRxSC_LOG(@"Subscribing to STOMP destination: %@", destination)
                    subscription = [self subscribeTo:destination headers:headers];
                    [_subscriptions setObject:subscription forKey:destination];
                } else {
                    LNRxSC_LOG(@"%lu subscribed to STOMP destination: %@", (unsigned long)subscription.subscribers, destination)
                }
            ...
            [[[self stompMessages]
              ...
              filter:^BOOL(LNStompMessage *message) {
                ...
                @synchronized(_subscriptions) {
                    ...
                    if (subscription) {
                        ...
                        if (subscription.subscribers <= 0) {
                            LNRxSC_LOG(@"Trying to unsubscribe from STOMP destination: %@", destination)
                            if ([self socketStateValid]) {
                                LNRxSC_LOG(@"Unsubscribing STOMP destination: %@", destination)
                                [subscription unsubscribe];
                            }
                            [_subscriptions removeObjectForKey:destination];
                        } else {
                            LNRxSC_LOG(@"%lu still subscribed to STOMP destination: %@", (unsigned long)subscription.subscribers, destination)
                        }
                    } else {
                        // shouldn't happen
                    }
                        ...
                        body:(NSString *)body
{
    ...
    LNStompFrame *frame = [[LNStompFrame alloc] initWithCommand:command headers:headers body:body];
    LNRxSC_LOG(@"Sending frame %@", frame)
    NSString *data = self.useSockJsFlag ? [frame toSockString] : [frame toString];
    [_socket send:data];
}

- (LNStompSubscription *)subscribeTo:(NSString *)destination
                              headers:(NSDictionary *)headers {
    ...
    [self sendFrameWithCommand:kCommandSubscribe
                       headers:subHeaders
                          body:nil];
    return [[LNStompSubscription alloc] initWithClient:self identifier:identifier];
}
...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message
{
    ...
    LNRxSC_LOG(@"received message: %@", extractedMessage)
    [self.socketSubject sendNext:extractedMessage];
}

- (void)webSocketDidOpen:(SRWebSocket *)webSocket
{
    LNRxSC_LOG(@"web socket opened")
    [self.socketSubject sendNext:webSocket];
}

- (void)webSocket:(SRWebSocket *)webSocket didFailWithError:(NSError *)error
{
    LNRxSC_LOG(@"web socket failed: %@", error)
...
{
    LNRxSC_LOG(@"web socket closed: code = %ld, reason = %@, clean ? %@", (long)code, reason, wasClean ? @"YES" : @"NO")
...
#pragma mark - LNStompFrame implementation

@implementation LNStompFrame
...
+ (LNStompFrame *)fromString:(NSString *)string
{
    ...
    return [[LNStompFrame alloc] initWithCommand:command headers:headers body:body];
}

@end

#pragma mark - LNStompSubscription implementation

@implementation LNStompMessage
...
+ (LNStompMessage *)fromFrame:(LNStompFrame *)frame
                        client:(LNStompClient *)client
{
    return [[LNStompMessage alloc] initWithClient:client
...
@implementation LNStompSubscription
